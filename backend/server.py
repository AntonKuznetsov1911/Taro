...
async def generate_ai_interpretation(question: str, category: str, spread_type: str, cards: List[TarotCard], positions: List[str]) -> str:
    ...
    prompt = f"""Ты профессиональный таролог с многолетним опытом. Проведи гадание на картах Таро на русском языке.
...
Инструкции:
1. {category_prompts.get(category, 'дай общую интерпретацию')}
2. Создай связное и глубокое толкование, объединяющее значения всех карт
3. Используй мистический, но понятный язык
4. Дай практические советы
5. Обязательно упоминай каждую карту и её позицию
6. Говори тепло и поддерживающе
7. Ответ должен быть 300-500 слов

Структура ответа:
- Общий обзор ситуации
- Анализ каждой позиции
- Объединяющее толкование
