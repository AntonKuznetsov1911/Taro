...
class TatoAiTester:
    ...
    def test_full_78_card_deck_completeness(self):
        """Test that all 78 cards are available in the system"""
        try:
            ...
            for i in range(15):  # Multiple readings to get good card coverage
                reading = self.test_create_reading("general", "celtic_cross", f"Тест полной колоды #{i+1}")
            ...
            good_coverage = total_unique_cards >= 30  # At least 30 different cards found
            
            details = f"Найдено {total_unique_cards} уникальных карт из {expected_total}. "
            details += f"Major Arcana: {major_arcana_found}, Wands: {wands_found}, Cups: {cups_found}, "
            details += f"Swords: {swords_found}, Pentacles: {pentacles_found}"
            
            if has_major and has_all_suits and good_coverage:
                self.log_test("Полнота колоды 78 карт", True, details)
                return True
            else:
                self.log_test("Полнота колоды 78 карт", False, f"Недостаточное покрытие колоды. {details}")
                return False
                
        except Exception as e:
            self.log_test("Полнота колоды 78 карт", False, f"Exception: {str(e)}")
            return False

    def test_card_variety_in_spreads(self):
        """Test that different card types appear in spreads"""
        try:
            # Test different spread types
            spread_tests = [
                ("celtic_cross", "love", "Разнообразие карт в кельтском кресте"),
                ("three_cards", "career", "Разнообразие карт в раскладе трех карт"),
                ("one_card", "finance", "Разнообразие карт в раскладе одной карты"),
                ("celtic_cross", "general", "Разнообразие карт в общем раскладе"),
                ("three_cards", "love", "Разнообразие карт в любовном раскладе")
            ]
            ...
            details = f"Всего карт: {total_cards}, Major: {major_count}, Minor: {minor_count}. "
            ...
            if has_major and has_minor and suits_present >= 3:
                self.log_test("Разнообразие карт в раскладах", True, details)
                return True
            else:
                self.log_test("Разнообразие карт в раскладах", False, f"Недостаточное разнообразие. {details}")
                return False
                
        except Exception as e:
            self.log_test("Разнообразие карт в раскладах", False, f"Exception: {str(e)}")
            return False

    def test_card_data_quality(self):
        """Test that all cards have proper data structure"""
        try:
            # Create readings to get card samples
            readings_data = []
            for i in range(5):
                reading = self.test_create_reading("general", "three_cards", f"Тест качества данных карт #{i+1}")
                if reading:
                    readings_data.append(reading)
            
            if not readings_data:
                self.log_test("Качество данных карт", False, "Не удалось создать чтения для тестирования")
                return False
            ...
            for reading in readings_data:
                for card in reading.get("cards", []):
                    ...
                    if not has_russian:
                        structure_issues.append(f"Карта {name} - имя не на русском языке")
            ...
            if len(structure_issues) <= 2 and image_quality >= 0.9:  # Allow minor issues
                self.log_test("Качество данных карт", True, details)
                return True
            else:
                details += f". Проблемы: {structure_issues[:3]}"  # Show first 3 issues
                self.log_test("Качество данных карт", False, details)
                return False
                
        except Exception as e:
            self.log_test("Качество данных карт", False, f"Exception: {str(e)}")
            return False

    def test_api_functionality_with_full_deck(self):
        """Test that all API functionality works with the full 78-card deck"""
        try:
            ...
            for spread_type, category, question in spread_tests:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        errors_found.append(f"{spread_type}/{category}: HTTP {response.status_code}")
            ...
            if success_rate >= 0.9:  # 90% success rate
                self.log_test("Функциональность API с полной колодой", True, details)
                return True
            else:
                details += f". Ошибки: {errors_found[:3]}"  # Show first 3 errors
                self.log_test("Функциональность API с полной колодой", False, details)
                return False
                
        except Exception as e:
            self.log_test("Функциональность API с полной колодой", False, f"Exception: {str(e)}")
            return False

    def test_minor_arcana_cards_presence(self):
        """Test specifically for Minor Arcana cards with ID > 21"""
        try:
            # Create multiple readings to find Minor Arcana cards
            minor_arcana_found = []
            cards_above_21 = []
            
            for i in range(20):  # More readings to increase chances
                reading = self.test_create_reading("general", "celtic_cross", f"Поиск Minor Arcana #{i+1}")
                if reading and "cards" in reading:
                    for card in reading["cards"]:
                        card_id = card.get("id")
            ...
            if unique_minor_cards:
                examples = list(unique_minor_cards.values())[:3]
                example_names = [f"{card['name']} (ID:{card['id']}, {card['suit']})" for card in examples]
                details += f". Примеры: {', '.join(example_names)}"
            
            if unique_minor_count >= 5 and len(suits_found) >= 2:  # At least 5 minor cards from 2+ suits
                self.log_test("Присутствие Minor Arcana (ID > 21)", True, details)
                return True
            else:
                self.log_test("Присутствие Minor Arcana (ID > 21)", False, f"Недостаточно Minor Arcana. {details}")
                return False
                
        except Exception as e:
            self.log_test("Присутствие Minor Arcana (ID > 21)", False, f"Exception: {str(e)}")
            return False

    def test_card_statistics_analysis(self):
        """Analyze card statistics across multiple readings"""
        try:
            ...
            test_scenarios = [
                ...
                ("celtic_cross", "finance"), ("three_cards", "general")
            ]
            
            for spread_type, category in test_scenarios:
                reading = self.test_create_reading(category, spread_type, f"Статистический анализ {spread_type}/{category}")
                if reading and "cards" in reading:
                    for card in reading["cards"]:
            ...
            # Most common cards
            most_common = sorted(card_counts.items(), key=lambda x: x[1], reverse=True)[:5]
            ...
            has_multiple_suits = sum(1 for count in suit_counts.values() if count > 0) >= 3
            good_variety = unique_cards >= 15
            
            if has_both_types and has_multiple_suits and good_variety:
                self.log_test("Статистический анализ карт", True, details)
                return True
            else:
                self.log_test("Статистический анализ карт", False, f"Недостаточное разнообразие. {details}")
                return False
                
        except Exception as e:
            self.log_test("Статистический анализ карт", False, f"Exception: {str(e)}")
            return False
    ...
    def run_deck_catalog_tests(self):
        """Run comprehensive deck catalog endpoint tests"""
        print("🃏 DECK CATALOG ENDPOINTS TESTING")
        print("=" * 50)
        print("Testing new deck catalog endpoints: /api/deck, /api/card/{id}, /api/card-interpretation")
        print()
        ...
        print("=" * 50)
        print(f"🃏 DECK CATALOG TESTING COMPLETE: {passed_tests}/{len(deck_tests)} tests passed ({(passed_tests/len(deck_tests))*100:.1f}%)")
        ...
        return passed_tests == len(deck_tests)
    ...
    def run_all_tests(self):
        ...
        # 🎯 NEW DECK CATALOG ENDPOINTS TESTING (as requested)
        print("\n🃏 TESTING NEW DECK CATALOG ENDPOINTS")
        print("-" * 50)
        self.run_deck_catalog_tests()
        print()
